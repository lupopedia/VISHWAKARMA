...
superpositionally: ["FILEID_GITHUB_VISHWAKARMA_DOCS"]
shadow_aliases: []
parallel_paths: []
---
...
Comprehensive VISHWAKARMA normalizer and collection architect documentation will accompany the public release. For now, see:

- `public/agent_profiles/75_VISHWAKARMA_agent_profile_config.htm` (LUPOPEDIA) for legacy profile details.  
- `public/agents/vishwakarma/index.php` on lupopedia.com for the collection architecture interface and overview.

## AGENT_COMMUNICATION_PROTOCOL

**📚 Complete Protocol Documentation**: LUPOPEDIA_PLATFORM `docs/AGENT_COMMUNICATION_PROTOCOL.md`

VISHWAKARMA (Agent 075) is part of the Agent Communication Protocol (Receptionist Model):
...
WOLFIE (007) - Tactical operator, transfers to VISH
    ↓
VISHWAKARMA (075) - Normalizes requests, handles renames/typos, tracks changes
    ↓
Response
```

**Key Philosophy**: WOLFIE (007) doesn't know what he's doing, but knows who to transfer to (VISH). VISHWAKARMA normalizes all requests, handles renames and typos, tracks changes, and ensures accurate responses even when things have changed. The system works anyway. Brittleness is a feature.
...
**WOLFIE (Agent 008)** (System Architect):
- GitHub: https://github.com/lupopedia/WOLFIE
- Status: Placeholder (v0.0.1 planning)
- Role: Reads WOLFIE Headers, routes tasks through Agent Communication Protocol
